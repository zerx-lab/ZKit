import type { en } from "./en";

// Chinese dictionary. Typed as `typeof en` so it must mirror the English shape
// key-for-key; a missing or extra key is a compile error. Mirror any future
// per-namespace split of `en.ts` here.
export const zh: typeof en = {
  app: { name: "ZKit", tagline: "全栈后台脚手架" },
  common: {
    signIn: "登录",
    signOut: "退出登录",
    signOutAll: "登出全部设备",
    register: "注册",
    email: "邮箱",
    password: "密码",
    name: "名称",
    role: "角色",
    roles: "角色",
    save: "保存",
    cancel: "取消",
    create: "创建",
    delete: "删除",
    edit: "编辑",
    search: "搜索",
    actions: "操作",
    id: "ID",
    created: "创建时间",
    loading: "加载中…",
    status: "状态",
    enabled: "启用",
    disabled: "禁用",
    description: "描述",
    sort: "排序",
    code: "编码",
    type: "类型",
    value: "值",
    label: "标签",
    key: "键",
    confirm: "确认",
    yes: "是",
    no: "否",
    add: "新增",
    refresh: "刷新",
    total: "共 {count} 条",
    previous: "上一页",
    next: "下一页",
    pagePrevGap: "向前 {count} 页",
    pageNextGap: "向后 {count} 页",
    pageSize: "每页条数",
    perPage: "{size} 条/页",
    jumpTo: "跳至",
    jumpUnit: "页",
    noData: "暂无数据。",
    nickname: "昵称",
    phone: "手机号",
    current: "当前",
    builtin: "内置",
    export: "导出",
    import: "导入",
    download: "下载",
    upload: "上传",
    run: "执行",
    expand: "展开",
    collapse: "收起",
    notFound: "页面未找到",
    error: "错误",
  },
  nav: {
    workspace: "工作台",
    dashboard: "仪表盘",
    users: "用户管理",
    management: "系统管理",
    system: "系统管理",
    audit: "审计日志",
    roles: "角色管理",
    menus: "菜单管理",
    apis: "接口管理",
    dicts: "字典管理",
    params: "参数管理",
    files: "文件管理",
    sessions: "会话管理",
    operationLogs: "操作日志",
    loginLogs: "登录日志",
    errorLogs: "错误日志",
    siteSettings: "网站设置",
    profile: "个人中心",
    jobs: "定时任务",
    plugins: "插件管理",
  },
  login: {
    title: "欢迎回来",
    subtitle: "登录以继续",
    noAccount: "还没有账号？",
    registerLink: "立即注册",
    submitting: "登录中…",
    failed: "登录失败",
    captchaLabel: "验证码",
    captchaPlaceholder: "请输入验证码",
    captchaRefresh: "刷新验证码",
    forgotPassword: "忘记密码？",
    totpLabel: "动态验证码",
    totpPlaceholder: "6 位动态码或恢复码",
    totpPrompt: "需要双因素认证",
  },
  register: {
    title: "创建账号",
    subtitle: "首个注册的账号将成为管理员",
    haveAccount: "已有账号？",
    loginLink: "去登录",
    submitting: "创建中…",
    failed: "注册失败",
    closed: "注册未开放",
    closedHint: "请联系管理员在网站设置中开启公开注册。",
  },
  dashboard: {
    overviewTitle: "系统概览",
    overviewDesc: "集中查看账号、访问、操作与认证状态。",
    period14: "近 14 天",
    totalUsers: "用户总数",
    totalRoles: "角色总数",
    activeSessions: "活跃会话",
    todayLogins: "今日登录",
    recentUsers: "近 14 天新增 {count} 个",
    roleCaption: "已配置的授权角色",
    sessionCaption: "当前已认证会话",
    todayLoginSummary: "成功 {success} 次 · 失败 {failure} 次",
    accessTrend: "访问与登录趋势",
    accessTrendDesc: "按日统计新增用户与登录结果",
    newUsers: "新增用户",
    loginSuccess: "登录成功",
    loginFailure: "登录失败",
    activitySummary: "活动摘要",
    activitySummaryDesc: "当前统计周期的聚合数据",
    successfulLogins: "成功登录",
    failedLogins: "失败登录",
    operations: "操作次数",
    currentSnapshot: "当前快照",
    operationTrend: "操作趋势",
    operationTrendDesc: "近 14 天每日审计操作次数",
    currentUser: "当前账号",
    accountSummaryDesc: "身份与授权信息摘要",
    accountId: "账号 ID",
    noPermission: "暂无仪表盘数据权限。",
  },
  users: {
    title: "用户管理",
    subtitle: "管理系统用户及其角色权限。",
    add: "新增用户",
    searchPlaceholder: "按名称搜索…",
    total: "共 {count} 条",
    noUsers: "暂无用户。",
    addTitle: "新增用户",
    addDesc: "创建一个新账号。",
    editTitle: "编辑用户",
    deleteTitle: "删除用户",
    deleteDesc: "确认永久删除 {email}？该操作不可撤销。",
    createdToast: "用户已创建",
    updatedToast: "用户已更新",
    deletedToast: "用户已删除",
    reset: "重置密码",
    resetTitle: "重置密码",
    resetDesc: "为 {email} 设置新密码，其所有会话将被注销。",
    resetToast: "密码已重置",
    newPassword: "新密码",
    exportToast: "导出中…",
    importTitle: "导入用户",
    importDesc: "上传 Excel 文件批量创建用户。",
    importBtn: "导入",
    importTemplate: "下载模板",
    importSuccess: "成功导入 {created} 位用户。",
    importFailures: "{count} 行失败：",
    importSelect: "选择 Excel 文件（.xlsx）",
    disableTotp: "关闭双因素认证",
    disableTotpToast: "已为用户关闭双因素认证",
    rolesLabel: "角色（可多选）",
  },
  roles: { admin: "超级管理员", user: "普通用户" },
  theme: { toggle: "切换主题" },
  validation: {
    email: "请输入有效的邮箱",
    passwordMin: "密码至少 8 位",
    nameRequired: "名称不能为空",
    required: "此项为必填",
    codePattern: "仅小写字母、数字和下划线，且以字母开头",
    rolesRequired: "请至少选择一个角色",
  },
  rolePage: {
    title: "角色管理",
    subtitle: "管理角色及其权限。",
    permissions: "权限",
    permissionsDescription: "选择该角色可访问的菜单、接口与操作按钮。",
    assign: "分配权限",
    assignTitle: "分配权限",
    savePermissions: "保存权限",
    tabMenus: "菜单权限",
    tabApis: "接口权限",
    tabButtons: "按钮权限",
    adminApiAccessTitle: "超级管理员拥有全部接口权限",
    adminApiAccessDescription:
      "系统会直接放行超级管理员，无需逐项授权或保存接口权限；菜单与按钮修改仍可正常保存。",
    allApisAccessible: "全部可访问",
    selectAllGroup: "全选本组",
    savedToast: "权限已保存",
    createdToast: "角色已创建",
    updatedToast: "角色已更新",
    deletedToast: "角色已删除",
  },
  menuPage: {
    title: "菜单管理",
    subtitle: "管理导航菜单与按钮权限。",
    parent: "父级",
    path: "路径",
    title_: "标题",
    icon: "图标",
    component: "组件",
    hidden: "隐藏",
    buttons: "按钮",
    manageButtons: "按钮管理",
    manageButtonsTitle: "{name} 的按钮",
    addButton: "新增按钮",
    searchPlaceholder: "搜索标题 / 路径",
    expandAll: "展开全部",
    collapseAll: "折叠全部",
    children: "{count} 项",
    createdToast: "菜单已创建",
    updatedToast: "菜单已更新",
    deletedToast: "菜单已删除",
    reorderedToast: "菜单顺序已更新",
    dragHandle: "拖拽排序",
    dragHint: "拖拽手柄可调整一级菜单及子菜单顺序。",
    pluginOwned: "插件 · {name}",
    pluginDisabled: "插件已禁用",
    enablePluginToManage: "启用插件后管理",
    disabledPluginHint: "已禁用插件的菜单会保留在配置目录中，以便重新启用时恢复原排序和权限；这些菜单不会出现在导航中，禁用期间不可编辑。",
  },
  apiPage: {
    title: "接口管理",
    subtitle: "RBAC 使用的 RPC 接口目录。",
    sync: "同步",
    syncedToast: "同步完成：新增 {added}，删除 {removed}",
    group: "分组",
    procedure: "接口",
    method: "方法",
    updatedToast: "接口已更新",
    deletedToast: "接口已删除",
    searchPlaceholder: "搜索接口 / 方法 / 描述",
    filterGroup: "分组",
    allGroups: "全部分组",
    expandAll: "展开全部",
    collapseAll: "折叠全部",
  },
  dictPage: {
    title: "字典管理",
    subtitle: "管理字典及其明细项。",
    items: "明细项",
    selectDict: "请选择左侧字典以查看明细。",
    addDict: "新增字典",
    addItem: "新增明细",
    createdToast: "字典已创建",
    updatedToast: "字典已更新",
    deletedToast: "字典已删除",
    itemCreatedToast: "明细已创建",
    itemUpdatedToast: "明细已更新",
    itemDeletedToast: "明细已删除",
  },
  paramPage: {
    title: "参数管理",
    subtitle: "管理运行时系统参数。",
    addParam: "新增参数",
    createdToast: "参数已创建",
    updatedToast: "参数已更新",
    deletedToast: "参数已删除",
  },
  filePage: {
    title: "文件管理",
    subtitle: "管理已上传的文件。",
    upload: "上传",
    uploading: "上传中…",
    uploadedToast: "文件已上传",
    deletedToast: "文件已删除",
    size: "大小",
    contentType: "类型",
    open: "打开",
    download: "下载",
    downloadFailed: "下载失败",
    searchPlaceholder: "按名称搜索…",
    viewList: "列表",
    viewGallery: "图廊",
    preview: "预览",
    previewImageAlt: "预览",
    previewTextLoading: "正在加载内容…",
    previewUnavailable: "该文件类型不支持预览。",
    previewLoadFailed: "预览加载失败。",
    visibility: "可见性",
    visPublic: "公开",
    visAuthenticated: "登录可见",
    visPrivate: "私有",
    visibilityHint: "公开文件任何人可访问；登录可见需登录后访问；私有文件仅上传者与管理员可见。",
    uploadTitle: "上传文件",
    uploadDialogDesc: "选择文件或文件夹，设置可见性，然后上传。",
    chooseFiles: "选择文件",
    chooseFolder: "选择文件夹",
    selectedFiles: "已选（{count}）",
    noFilesSelected: "尚未选择文件。",
    removeFile: "移除",
    clearAll: "清空",
    uploadProgress: "上传中 {done}/{total}…",
    uploadDoneToast: "已上传 {count} 个文件",
    uploadPartialToast: "已上传 {ok}/{total}，{failed} 个失败",
    startUpload: "上传",
  },
  sitePage: {
    title: "网站设置",
    subtitle: "设置网站标识、公开注册开关与新用户默认角色。",
    name: "网站名称",
    namePlaceholder: "请输入网站名称",
    logo: "Logo 地址",
    logoPlaceholder: "请输入或上传 Logo 地址",
    uploadLogo: "上传",
    uploading: "上传中…",
    domain: "访问域名",
    domainPlaceholder: "例如 example.com",
    registerEnabled: "开放注册",
    registerEnabledHint: "默认开启。关闭后将隐藏公开注册入口。",
    registerDefaultRole: "新注册用户默认角色",
    registerDefaultRoleHint: "仅在已有管理员之后生效，不可选择管理员。",
    save: "保存",
    saving: "保存中…",
    savedToast: "网站设置已保存",
  },
  sessionPage: {
    title: "会话管理",
    subtitle: "当前活跃的登录会话。",
    ip: "IP",
    device: "设备",
    lastSeen: "最近活跃",
    revoke: "下线",
    revokedToast: "会话已下线",
    userIdLabel: "用户 ID（管理员）",
    apply: "应用",
  },
  logPage: {
    operationTitle: "操作日志",
    loginTitle: "登录日志",
    errorTitle: "错误日志",
    subtitle: "审计记录。",
    procedure: "接口",
    ip: "IP",
    latency: "耗时 (ms)",
    status: "状态",
    error: "错误",
    success: "成功",
    failure: "失败",
    clean: "清理",
    cleanDays: "保留天数",
    cleanedToast: "已清理 {count} 条日志",
    searchPlaceholder: "搜索…",
    filterStatus: "状态筛选",
    filterMethod: "请求方法",
    filterStartDate: "开始日期",
    filterEndDate: "结束日期",
    filterSuccess: "结果",
    allStatuses: "全部状态",
    allResults: "全部",
    statusOk: "OK",
    methodPlaceholder: "如 POST",
    detailTitle: "详情",
    exportToast: "导出中…",
  },
  profile: {
    title: "个人中心",
    subtitle: "管理你的账号信息与安全设置。",
    profileSection: "个人信息",
    profileDesc: "更新你的昵称、头像和手机号。",
    avatarLabel: "头像地址",
    avatarUpload: "上传头像",
    savedToast: "个人资料已保存",
    passwordSection: "修改密码",
    passwordDesc: "修改密码后，你的所有会话将被注销。",
    oldPassword: "当前密码",
    newPassword: "新密码",
    confirmPassword: "确认新密码",
    passwordChanged: "密码已修改，请重新登录。",
    passwordMismatch: "两次密码不一致",
    twoFactorSection: "双因素认证",
    twoFactorDesc: "使用身份验证器 App 保护你的账号。",
    twoFactorEnabled: "双因素认证已启用",
    twoFactorDisabled: "双因素认证未启用",
    enableBtn: "启用双因素认证",
    disableBtn: "关闭双因素认证",
    setupStep1: "请用验证器 App 扫描二维码。",
    setupStep2: "输入 6 位验证码以完成激活。",
    secretLabel: "手动输入密钥",
    activateBtn: "激活",
    activatedToast: "双因素认证已启用",
    disabledToast: "双因素认证已关闭",
    recoveryCodes: "恢复码",
    recoveryCodesDesc: "请将以下恢复码保存到安全的地方，每个只能使用一次。",
    disableCodeLabel: "输入当前验证码以关闭",
  },
  pluginPage: {
    title: "插件管理",
    subtitle: "已编译进二进制的插件。启用/禁用为运行时开关;安装/卸载需重新构建。",
    installHint: "安装/卸载需重新构建:用 `task new-plugin` 生成骨架,再 `task gen` + `task build` 并重启。此处禁用会在运行时隐藏插件菜单、拒绝其接口、跳过其任务。",
    name: "名称",
    services: "服务",
    tables: "数据表",
    menus: "菜单",
    jobs: "任务",
    enabled: "启用",
    on: "已启用",
    off: "已禁用",
    empty: "没有已编译的插件。",
    toggled: "插件状态已更新",
    publicPages: "公开页",
    group: "分组",
    dataRetained: "禁用只关闭插件的菜单、接口与任务,其数据表与数据完整保留;重新启用即可恢复对同一批数据的访问。",
    upload: "上传插件",
    installed: "插件 {name} 已安装。",
    uninstalled: "插件已卸载。",
    uninstall: "卸载",
    uninstallTitle: "卸载插件 {name}?",
    uninstallDesc: "将移除该插件的代码与菜单。请选择是否同时 DROP 其数据表并删除全部数据。",
    uninstallPurgeWarn: "DROP 数据表不可逆 —— 该插件数据表中的所有数据将被永久删除。",
    uninstallPurge: "删除数据表与数据",
    uninstallKeep: "保留数据",
    pendingDev: "已写入源码并重新生成代码。请重启后端服务(dev 下在 mprocs 中重启 backend 进程)以生效。",
    pendingProd: "已写入源码。请在构建环境执行 task gen && task build 并重新部署以生效。",
    pendingRemoval: "待重启移除",
    restarting: "已提交。服务正在重新编译/重启,请稍后刷新查看结果。",
  },
  jobs: {
    title: "定时任务",
    subtitle: "管理和监控定时调度的后台任务。",
    add: "新增任务",
    addTitle: "新增任务",
    editTitle: "编辑任务",
    deleteTitle: "删除任务",
    deleteDesc: "确认删除任务 '{name}'？",
    createdToast: "任务已创建",
    updatedToast: "任务已更新",
    deletedToast: "任务已删除",
    runToast: "任务已触发",
    runBtn: "立即执行",
    nameLabel: "任务名称",
    handlerLabel: "处理器",
    handlerPlaceholder: "请选择处理器",
    cronLabel: "Cron 表达式",
    cronPlaceholder: "如 0 3 * * *",
    enabledLabel: "启用",
    descriptionLabel: "描述",
    lastRunAt: "上次执行",
    history: "执行历史",
    historyTitle: "{name} 的执行历史",
    execStatus: "状态",
    execStarted: "开始时间",
    execDuration: "耗时 (ms)",
    execError: "错误",
    noJobs: "暂无任务。",
    noHistory: "暂无执行记录。",
  },
  passwordReset: {
    forgotTitle: "忘记密码",
    forgotSubtitle: "输入邮箱以接收重置链接。",
    forgotSubmit: "发送重置链接",
    forgotSubmitting: "发送中…",
    forgotSuccess: "如果该邮箱已注册，重置链接已发送，请查收邮件。",
    backToLogin: "返回登录",
    resetTitle: "设置新密码",
    resetSubtitle: "请输入新密码。",
    resetSubmit: "设置密码",
    resetSubmitting: "设置中…",
    resetSuccess: "密码设置成功，请使用新密码登录。",
    tokenLabel: "令牌",
    newPassword: "新密码",
  },
  totp: {
    label: "双因素验证码",
    placeholder: "请输入 6 位验证码",
  },
  notFound: {
    title: "页面不存在",
    description: "您访问的页面不存在或已被移动。",
    home: "返回首页",
  },
  errorPage: {
    title: "出错了",
    forbidden: "您没有权限访问该页面。",
    internal: "服务器发生错误，请稍后重试。",
    retry: "重试",
    home: "返回首页",
  },
};
