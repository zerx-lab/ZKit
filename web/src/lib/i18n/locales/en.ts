// English dictionary. This is the canonical shape — `zh` and any future locale
// must satisfy `typeof en`, so adding a key here makes it required everywhere.
// As features grow, split this object into per-namespace files under
// `locales/en/` and re-export a merged object from here; `zh.ts` mirrors it.
export const en = {
  app: { name: "ZKit", tagline: "Full-stack admin starter" },
  common: {
    signIn: "Sign in",
    signOut: "Sign out",
    signOutAll: "Sign out all devices",
    register: "Register",
    email: "Email",
    password: "Password",
    name: "Name",
    role: "Role",
    roles: "Roles",
    save: "Save",
    cancel: "Cancel",
    create: "Create",
    delete: "Delete",
    edit: "Edit",
    search: "Search",
    actions: "Actions",
    id: "ID",
    created: "Created",
    loading: "Loading…",
    status: "Status",
    enabled: "Enabled",
    disabled: "Disabled",
    description: "Description",
    sort: "Sort",
    code: "Code",
    type: "Type",
    value: "Value",
    label: "Label",
    key: "Key",
    confirm: "Confirm",
    yes: "Yes",
    no: "No",
    add: "Add",
    refresh: "Refresh",
    total: "{count} total",
    previous: "Previous",
    next: "Next",
    pagePrevGap: "Back {count} pages",
    pageNextGap: "Forward {count} pages",
    pageSize: "Page size",
    perPage: "{size} / page",
    jumpTo: "Go to",
    jumpUnit: "page",
    noData: "No data.",
    nickname: "Nickname",
    phone: "Phone",
    current: "current",
    builtin: "Built-in",
    export: "Export",
    import: "Import",
    download: "Download",
    upload: "Upload",
    run: "Run",
    expand: "Expand",
    collapse: "Collapse",
    notFound: "Page not found",
    error: "Error",
  },
  nav: {
    workspace: "Workspace",
    dashboard: "Dashboard",
    users: "Users",
    management: "Management",
    system: "System",
    audit: "Audit",
    roles: "Roles",
    menus: "Menus",
    apis: "APIs",
    dicts: "Dictionaries",
    params: "Parameters",
    files: "Files",
    sessions: "Sessions",
    operationLogs: "Operation Logs",
    loginLogs: "Login Logs",
    errorLogs: "Error Logs",
    siteSettings: "Site Settings",
    profile: "Profile",
    jobs: "Scheduled Jobs",
    plugins: "Plugins",
  },
  login: {
    title: "Welcome back",
    subtitle: "Sign in to continue",
    noAccount: "Don't have an account?",
    registerLink: "Create one",
    submitting: "Signing in…",
    failed: "Login failed",
    captchaLabel: "Captcha",
    captchaPlaceholder: "Enter the code",
    captchaRefresh: "Refresh captcha",
    forgotPassword: "Forgot password?",
    totpLabel: "Authenticator code",
    totpPlaceholder: "6-digit code or recovery code",
    totpPrompt: "Two-factor authentication required",
  },
  register: {
    title: "Create your account",
    subtitle: "The first account becomes the administrator",
    haveAccount: "Already have an account?",
    loginLink: "Sign in",
    submitting: "Creating…",
    failed: "Registration failed",
    closed: "Registration is closed",
    closedHint: "Ask an administrator to enable public registration in site settings.",
  },
  dashboard: {
    overviewTitle: "System overview",
    overviewDesc: "Monitor accounts, access, activity, and authentication from one place.",
    period14: "Last 14 days",
    totalUsers: "Total users",
    totalRoles: "Total roles",
    activeSessions: "Active sessions",
    todayLogins: "Today's logins",
    recentUsers: "{count} new in the last 14 days",
    roleCaption: "Configured authorization roles",
    sessionCaption: "Currently authenticated sessions",
    todayLoginSummary: "{success} successful · {failure} failed",
    accessTrend: "Access and login trends",
    accessTrendDesc: "Daily new users and login outcomes",
    newUsers: "New users",
    loginSuccess: "Successful logins",
    loginFailure: "Failed logins",
    activitySummary: "Activity summary",
    activitySummaryDesc: "Aggregate activity for the selected period",
    successfulLogins: "Successful logins",
    failedLogins: "Failed logins",
    operations: "Operations",
    currentSnapshot: "Current snapshot",
    operationTrend: "Operation trend",
    operationTrendDesc: "Daily audited operations over the last 14 days",
    currentUser: "Current account",
    accountSummaryDesc: "Identity and authorization summary",
    accountId: "Account ID",
    noPermission: "No dashboard data permission.",
  },
  users: {
    title: "Users",
    subtitle: "Manage system users and their roles.",
    add: "Add user",
    searchPlaceholder: "Search by name…",
    total: "{count} total",
    noUsers: "No users found.",
    addTitle: "Add user",
    addDesc: "Create a new account.",
    editTitle: "Edit user",
    deleteTitle: "Delete user",
    deleteDesc: "Permanently remove {email}? This action cannot be undone.",
    createdToast: "User created",
    updatedToast: "User updated",
    deletedToast: "User deleted",
    reset: "Reset password",
    resetTitle: "Reset password",
    resetDesc: "Set a new password for {email}. Their sessions will be revoked.",
    resetToast: "Password reset",
    newPassword: "New password",
    exportToast: "Exporting…",
    importTitle: "Import users",
    importDesc: "Upload an Excel file to batch-create users.",
    importBtn: "Import",
    importTemplate: "Download template",
    importSuccess: "Imported {created} user(s).",
    importFailures: "{count} row(s) failed:",
    importSelect: "Select Excel file (.xlsx)",
    disableTotp: "Disable 2FA",
    disableTotpToast: "2FA disabled for user",
    rolesLabel: "Roles (select one or more)",
  },
  roles: { admin: "Super Admin", user: "User" },
  theme: { toggle: "Toggle theme" },
  validation: {
    email: "Enter a valid email",
    passwordMin: "Password must be at least 8 characters",
    nameRequired: "Name is required",
    required: "This field is required",
    codePattern: "Lowercase letters, digits and underscores; must start with a letter",
    rolesRequired: "Select at least one role",
  },
  rolePage: {
    title: "Roles",
    subtitle: "Manage roles and their permissions.",
    permissions: "Permissions",
    permissionsDescription: "Choose the menus, APIs, and action buttons this role can access.",
    assign: "Assign permissions",
    assignTitle: "Assign permissions",
    savePermissions: "Save permissions",
    tabMenus: "Menu access",
    tabApis: "API access",
    tabButtons: "Button access",
    adminApiAccessTitle: "Super admins can access every API",
    adminApiAccessDescription:
      "No API grants need to be selected or saved; menu and button changes can still be saved.",
    allApisAccessible: "All accessible",
    selectAllGroup: "Select all",
    savedToast: "Permissions saved",
    createdToast: "Role created",
    updatedToast: "Role updated",
    deletedToast: "Role deleted",
  },
  menuPage: {
    title: "Menus",
    subtitle: "Manage navigation and button permissions.",
    parent: "Parent",
    path: "Path",
    title_: "Title",
    icon: "Icon",
    component: "Component",
    hidden: "Hidden",
    buttons: "Buttons",
    manageButtons: "Buttons",
    manageButtonsTitle: "Buttons of {name}",
    addButton: "Add button",
    searchPlaceholder: "Search title / path",
    expandAll: "Expand all",
    collapseAll: "Collapse all",
    children: "{count} items",
    createdToast: "Menu created",
    updatedToast: "Menu updated",
    deletedToast: "Menu deleted",
    reorderedToast: "Menu order updated",
    dragHandle: "Drag to reorder",
    dragHint: "Drag the handle to reorder top-level groups and their children.",
    pluginOwned: "Plugin · {name}",
    pluginDisabled: "Plugin disabled",
    enablePluginToManage: "Enable plugin to manage",
    disabledPluginHint: "Menus from disabled plugins stay in this catalog so their order and permissions can be restored when re-enabled. They are hidden from navigation, and editing is locked while the plugin is disabled.",
  },
  apiPage: {
    title: "APIs",
    subtitle: "Catalog of RPC procedures used by RBAC.",
    sync: "Sync",
    syncedToast: "Synced: {added} added, {removed} removed",
    group: "Group",
    procedure: "Procedure",
    method: "Method",
    updatedToast: "API updated",
    deletedToast: "API deleted",
    searchPlaceholder: "Search procedure / method / description",
    filterGroup: "Group",
    allGroups: "All groups",
    expandAll: "Expand all",
    collapseAll: "Collapse all",
  },
  dictPage: {
    title: "Dictionaries",
    subtitle: "Manage lookup dictionaries and their items.",
    items: "Items",
    selectDict: "Select a dictionary to view its items.",
    addDict: "Add dictionary",
    addItem: "Add item",
    createdToast: "Dictionary created",
    updatedToast: "Dictionary updated",
    deletedToast: "Dictionary deleted",
    itemCreatedToast: "Item created",
    itemUpdatedToast: "Item updated",
    itemDeletedToast: "Item deleted",
  },
  paramPage: {
    title: "Parameters",
    subtitle: "Manage runtime system parameters.",
    addParam: "Add parameter",
    createdToast: "Parameter created",
    updatedToast: "Parameter updated",
    deletedToast: "Parameter deleted",
  },
  filePage: {
    title: "Files",
    subtitle: "Manage uploaded files.",
    upload: "Upload",
    uploading: "Uploading…",
    uploadedToast: "File uploaded",
    deletedToast: "File deleted",
    size: "Size",
    contentType: "Type",
    open: "Open",
    download: "Download",
    downloadFailed: "Download failed",
    searchPlaceholder: "Search by name…",
    viewList: "List",
    viewGallery: "Gallery",
    preview: "Preview",
    previewImageAlt: "Preview",
    previewTextLoading: "Loading content…",
    previewUnavailable: "Preview not available for this file type.",
    previewLoadFailed: "Failed to load preview.",
    visibility: "Visibility",
    visPublic: "Public",
    visAuthenticated: "Signed-in",
    visPrivate: "Private",
    visibilityHint: "Public files are reachable by anyone; signed-in files require login; private files are visible only to the uploader and admins.",
    uploadTitle: "Upload files",
    uploadDialogDesc: "Pick files or a folder, choose visibility, then upload.",
    chooseFiles: "Choose files",
    chooseFolder: "Choose folder",
    selectedFiles: "Selected ({count})",
    noFilesSelected: "No files selected yet.",
    removeFile: "Remove",
    clearAll: "Clear",
    uploadProgress: "Uploading {done}/{total}…",
    uploadDoneToast: "Uploaded {count} file(s)",
    uploadPartialToast: "Uploaded {ok}/{total}; {failed} failed",
    startUpload: "Upload",
  },
  sitePage: {
    title: "Site Settings",
    subtitle: "Configure site identity, public registration, and the default role.",
    name: "Site Name",
    namePlaceholder: "Enter the site name",
    logo: "Logo URL",
    logoPlaceholder: "Enter or upload a logo URL",
    uploadLogo: "Upload",
    uploading: "Uploading…",
    domain: "Access Domain",
    domainPlaceholder: "e.g. example.com",
    registerEnabled: "Public registration",
    registerEnabledHint: "On by default. Turn off to hide the public register entry.",
    registerDefaultRole: "Default role for new registrations",
    registerDefaultRoleHint: "Applied after the first admin exists. Admin cannot be selected.",
    save: "Save",
    saving: "Saving…",
    savedToast: "Site settings saved",
  },
  sessionPage: {
    title: "Sessions",
    subtitle: "Active login sessions.",
    ip: "IP",
    device: "Device",
    lastSeen: "Last seen",
    revoke: "Revoke",
    revokedToast: "Session revoked",
    userIdLabel: "User ID (admin)",
    apply: "Apply",
  },
  logPage: {
    operationTitle: "Operation Logs",
    loginTitle: "Login Logs",
    errorTitle: "Error Logs",
    subtitle: "Audit trail.",
    procedure: "Procedure",
    ip: "IP",
    latency: "Latency (ms)",
    status: "Status",
    error: "Error",
    success: "Success",
    failure: "Failure",
    clean: "Clean",
    cleanDays: "Days to keep",
    cleanedToast: "{count} logs removed",
    searchPlaceholder: "Search…",
    filterStatus: "Status filter",
    filterMethod: "Method",
    filterStartDate: "Start date",
    filterEndDate: "End date",
    filterSuccess: "Result",
    allStatuses: "All statuses",
    allResults: "All",
    statusOk: "OK",
    methodPlaceholder: "e.g. POST",
    detailTitle: "Detail",
    exportToast: "Exporting…",
  },
  profile: {
    title: "Profile",
    subtitle: "Manage your account information and security settings.",
    profileSection: "Personal information",
    profileDesc: "Update your display name, avatar, and phone number.",
    avatarLabel: "Avatar URL",
    avatarUpload: "Upload avatar",
    savedToast: "Profile updated",
    passwordSection: "Change password",
    passwordDesc: "After changing your password all your sessions will be revoked.",
    oldPassword: "Current password",
    newPassword: "New password",
    confirmPassword: "Confirm new password",
    passwordChanged: "Password changed. Please sign in again.",
    passwordMismatch: "Passwords do not match",
    twoFactorSection: "Two-factor authentication",
    twoFactorDesc: "Protect your account with an authenticator app.",
    twoFactorEnabled: "2FA is enabled",
    twoFactorDisabled: "2FA is not enabled",
    enableBtn: "Enable 2FA",
    disableBtn: "Disable 2FA",
    setupStep1: "Scan the QR code with your authenticator app.",
    setupStep2: "Enter the 6-digit code to verify and activate.",
    secretLabel: "Manual entry key",
    activateBtn: "Activate",
    activatedToast: "2FA enabled",
    disabledToast: "2FA disabled",
    recoveryCodes: "Recovery codes",
    recoveryCodesDesc: "Save these codes in a secure place. Each can be used once.",
    disableCodeLabel: "Enter your current 2FA code to disable",
  },
  pluginPage: {
    title: "Plugin Management",
    subtitle: "Compiled-in plugins. Enable/disable is a runtime toggle; install/uninstall needs a rebuild.",
    installHint: "Install/uninstall requires a rebuild: scaffold with `task new-plugin`, then `task gen` + `task build` and restart. Disabling here hides a plugin's menus, denies its APIs, and skips its jobs at runtime.",
    name: "Name",
    services: "Services",
    tables: "Tables",
    menus: "Menus",
    jobs: "Jobs",
    enabled: "Enabled",
    on: "On",
    off: "Off",
    empty: "No plugins compiled in.",
    toggled: "Plugin state updated",
    publicPages: "Public Pages",
    group: "group",
    dataRetained: "Disabling keeps the plugin's tables and data intact — only its menus, APIs and jobs are turned off. Re-enabling restores full access to the same data.",
    upload: "Upload plugin",
    installed: "Plugin {name} installed.",
    uninstalled: "Plugin uninstalled.",
    uninstall: "Uninstall",
    uninstallTitle: "Uninstall plugin {name}?",
    uninstallDesc: "This removes the plugin's code and menus. Choose whether to also drop its tables and delete all its data.",
    uninstallPurgeWarn: "Dropping tables is irreversible — all data in this plugin's tables will be permanently deleted.",
    uninstallPurge: "Drop tables & delete data",
    uninstallKeep: "Keep data",
    pendingDev: "Source written and code regenerated. Restart the backend (in dev: restart the backend process in mprocs) to apply.",
    pendingProd: "Source written. Run task gen && task build in a build environment and redeploy to take effect.",
    pendingRemoval: "Pending restart (removed)",
    restarting: "Submitted. The server is rebuilding/restarting — refresh in a moment to see the result.",
  },
  jobs: {
    title: "Scheduled Jobs",
    subtitle: "Manage and monitor cron-scheduled background tasks.",
    add: "Add job",
    addTitle: "Add job",
    editTitle: "Edit job",
    deleteTitle: "Delete job",
    deleteDesc: "Permanently remove job '{name}'?",
    createdToast: "Job created",
    updatedToast: "Job updated",
    deletedToast: "Job deleted",
    runToast: "Job triggered",
    runBtn: "Run now",
    nameLabel: "Name",
    handlerLabel: "Handler",
    handlerPlaceholder: "Select a handler",
    cronLabel: "Cron expression",
    cronPlaceholder: "e.g. 0 3 * * *",
    enabledLabel: "Enabled",
    descriptionLabel: "Description",
    lastRunAt: "Last run",
    history: "History",
    historyTitle: "Execution history: {name}",
    execStatus: "Status",
    execStarted: "Started",
    execDuration: "Duration (ms)",
    execError: "Error",
    noJobs: "No jobs found.",
    noHistory: "No executions yet.",
  },
  passwordReset: {
    forgotTitle: "Forgot password",
    forgotSubtitle: "Enter your email to receive a reset link.",
    forgotSubmit: "Send reset link",
    forgotSubmitting: "Sending…",
    forgotSuccess: "If an account with that email exists, a reset link has been sent.",
    backToLogin: "Back to sign in",
    resetTitle: "Set new password",
    resetSubtitle: "Enter your new password below.",
    resetSubmit: "Set password",
    resetSubmitting: "Setting…",
    resetSuccess: "Password set successfully. You can now sign in.",
    tokenLabel: "Token",
    newPassword: "New password",
  },
  totp: {
    label: "Two-factor code",
    placeholder: "Enter 6-digit code",
  },
  notFound: {
    title: "Page not found",
    description: "The page you are looking for does not exist or has been moved.",
    home: "Back to home",
  },
  errorPage: {
    title: "Something went wrong",
    forbidden: "You do not have permission to access this page.",
    internal: "The server encountered an error. Please try again later.",
    retry: "Retry",
    home: "Back to home",
  },
};
